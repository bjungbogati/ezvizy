...
server <- function(input, output, session) {
  ...
  # observeEvent(values$list, {
  #   
  #   # values$list <- purrr::compact(values$list)
  #   # values$list <- stringr::str_replace_all(values$list,"''", NULL)
  #   })
  ...
  observeEvent(input$submitgroup, {
    if (input$submitgroup > 0) {
      ...
      # values$list <- lapply(values$list, function(x) x[!x %in% ""])
  ...
  observeEvent(input$cleargroup, {
    if (input$cleargroup > 0) {
      ...
      # values$list <- lapply(values$list, function(x) x[!x %in% input$grouping]) 
